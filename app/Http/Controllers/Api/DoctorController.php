...
class DoctorController extends Controller
{
    ...
    // QUERY FOR DOCTORS SEARCH BY MIN REVIEWS NUMBER
    public function searchByReview(int $review)
    {
        $doctors = Doctor::with('user', 'specializations')->withCount('reviews')->withAvg('votes', 'vote')->having('reviews_count', '>=', $review)->with(
            ['sponsorships' => function ($item) {
                return $item->where('end_date', '>=', date('Y-m-d H:i:s'));
            }]
        )->get();
